...
def main():
    try:
        ...
        cities = load_cities(WORLD_CITIES_FILE)#loads and select the random city
        ...
        # Fetch weather data
        weather_data = fetch_weather_data(lat=lat, lon=lon)#get the 30 days weather
        if not weather_data:
            raise ValueError("No weather data returned from the API.")

        # Train clustering model
        avg_temps = [day["max_temp"] for day in weather_data]  # Changed to max_temp for consistency
